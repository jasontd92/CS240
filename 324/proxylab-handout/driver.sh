...
if [ $MODE != "multiprocess" -a $num_processes_realtime -gt $num_processes_pre ]; then
	echo "    Error: Processes are being forked on the fly: $num_processes_realtime > $num_processes_pre"
fi
if [ $MODE != "processpool" -a $num_processes_pre -gt 1 ]; then
	echo "    Error: A pool of $(( $num_processes_pre - 1 )) was created a program start."
fi
if [ $MODE != "multithread" -a $num_threads_realtime -gt $num_threads_pre ]; then
	echo "    Error: Threads are being spawned on the fly: $num_threads_realtime > $num_threads_pre"
fi
if [ $MODE != "threadpool" -a $num_threads_pre -gt 2 ]; then # account for logging thread (i.e., 2 at start)
	echo "    Error: A pool of $(( $num_threads_pre - 1 )) was created at program start."
fi
...
if [ $MODE == "epoll" ] && ! echo -e "$fd_output" | awk '{ print $10 }' | grep -q eventpoll; then
	echo "    Error: No active epoll file descriptor detected"
fi
listenfd_flags=`echo -e "$fd_output" | awk '$9 == "TCP" && $10 ~ /:'$fd_output_proxy_port'$/ && $11 ~ /LISTEN/ { print $6 }'`
if [ -z "$listenfd_flags" ]; then
	echo "    Warning: Listening file descriptor could not be detected."
elif echo -e "$listenfd_flags" | grep -vq 'NB\|ND'; then
	if [ $MODE == "epoll" ]; then
		echo "    Error: Listening file descriptor is *not* configured as non-blocking"
	fi
else
	if [ $MODE != "epoll" ]; then
		echo "    Error: Listening file descriptor is configured as non-blocking"
	fi
fi
clientfd_flags=`echo -e "$fd_output" | awk '$9 == "TCP" && $10 ~/:'$fd_output_proxy_port'(->|$)/ { print $6 }'`
if [ -z "$clientfd_flags" ]; then
	echo "    Warning: Client-facing file descriptors could not be detected."
elif echo -e "$clientfd_flags" | grep -vq 'NB\|ND'; then
	if [ $MODE == "epoll" ]; then
		echo "    Error: Client-facing file descriptors are *not* configured as non-blocking"
	fi
else
	if [ $MODE != "epoll" ]; then
		echo "    Error: Client-facing file descriptors are configured as non-blocking"
	fi
fi
serverfd_flags=`echo -e "$fd_output" | awk '$9 == "TCP" && $10 ~/:'$fd_output_tiny_port'(->|$)/ { print $6 }'`
if [ -z "$serverfd_flags" ]; then
	echo "    Warning: Server-facing file descriptors could not be detected."
elif echo -e "$serverfd_flags" | grep -vq 'NB\|ND'; then
	if [ $MODE == "epoll" ]; then
		echo "    Error: Server-facing file descriptors are *not* configured as non-blocking"
	fi
else
	if [ $MODE != "epoll" ]; then
		echo "    Error: Server-facing file descriptors are configured as non-blocking"
	fi
fi
